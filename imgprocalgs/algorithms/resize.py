...
class ImageResizer(BaseImageAlgorithm, metaclass=abc.ABCMeta):
    """Base class for resizing algorithms"""
    ...
    def process(self):
        """
        Required by BaseImageAlgorithm.
        Concrete resize algorithms should use process_with_output.
        """
        raise NotImplementedError("Use process_with_output(dest_path) instead")

    @abc.abstractmethod
    def process_with_output(self, dest_path: str):
        pass

    def get_new_image(self, width, height):
    ...
    def process_with_output(self, dest_path: str):
        ...
        for x in range(x_dest):
            for y in range(y_dest):
                ...
                red = (
                    (1 - dy) * (1 - dx) * self.neigh1.red
                    + (1 - dy) * dx * self.neigh2.red
                    + (1 - dx) * dy * self.neigh3.red
                    + dx * dy * self.neigh4.red
                )
                green = (
                    (1 - dy) * (1 - dx) * self.neigh1.green
                    + (1 - dy) * dx * self.neigh2.green
                    + (1 - dx) * dy * self.neigh3.green
                    + dx * dy * self.neigh4.green
                )
                blue = (
                    (1 - dy) * (1 - dx) * self.neigh1.blue
                    + (1 - dy) * dx * self.neigh2.blue
                    + (1 - dx) * dy * self.neigh3.blue
                    + dx * dy * self.neigh4.blue
                )
    ...
    def process_with_output(self, dest_path: str):
        ...
        for x in range(x_dest):
            for y in range(y_dest):
                ...
                dx, dy = x0 - x1, y0 - y1
                self.x_coofs = self.get_coefficients(dx)
                self.y_coofs = self.get_coefficients(dy)
                    ...
                    for nx in (x1, x2):
                        for _ in range(4):
                            self.neighbours.append(
                                Neighbour(nx, ny, self.image.pixels[nx, ny])
                            )

                red = self._interpolate("red")
                green = self._interpolate("green")
                blue = self._interpolate("blue")
...
# Demo helpers (fixed to use process_with_output)

neigbhour_2 = functools.partial(
    NearestNeigbhour.process_with_output,
    NearestNeigbhour("tests/data/bird.jpg", 2),
    "data/n_2.jpg"
)

neigbhour_4 = functools.partial(
    NearestNeigbhour.process_with_output,
    NearestNeigbhour("tests/data/bird.jpg", 4),
    "data/n_4.jpg"
)

bilinear_2 = functools.partial(
    BilinearInterpolation.process_with_output,
    BilinearInterpolation("tests/data/bird.jpg", 2),
    "data/bl_2.jpg"
)

bilinear_4 = functools.partial(
    BilinearInterpolation.process_with_output,
    BilinearInterpolation("tests/data/bird.jpg", 4),
    "data/bl_4.jpg"
)

bicubic_2 = functools.partial(
    BicubicInterpolation.process_with_output,
    BicubicInterpolation("tests/data/bird.jpg", 2),
    "data/bc_2.jpg"
)

bicubic_4 = functools.partial(
    BicubicInterpolation.process_with_output,
    BicubicInterpolation("tests/data/bird.jpg", 4),
    "data/bc_4.jpg"
)
